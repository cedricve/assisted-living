...
public:
	...
	// 
	// This function uses the TOP-CAMERA
	static Vec3f findCoffeeHolder(const Mat & img, Mat& result, CoffeeMakerPosition & pos){
		Mat blur;

		// Crop the image, uses information of the calibration
		// This function cuts out the yellow tape of the waterreservoir
		Mat cropped;
		cropped = Helper::crop(img, Rect(Point(0,0),Point(img.cols,pos.getY()-pos.getRatio()*100)));
	
		// Filter image to extract only yellow
		...
		// Closing operation to get a better circle
		...
		// Specify a range, where the coffeeholder has to be searched for (y-coordinate)
		// in this situation only y-coordinates for 0=>200 are allowed. 
		// You can chnage it to img.rows if the coffeeholder can be placed anywhere in the viewport of the
		// top camera
		int offset = 200;

		// Use the houghcircles function to detect
	...
	static int getTypeFilter(const Mat & gray, Mat & result, const Vec3f & middle, const int fault){
		...
		// scan a small region around the position of the coffeeholder
		// calculate the arithmetic mean of the grayscale intensities
